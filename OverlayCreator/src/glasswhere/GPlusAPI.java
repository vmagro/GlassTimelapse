...
public class GPlusAPI {
	...
	public static String getAuthToken(String glassid) throws IOException {
		...
		HttpPost httpPost = new HttpPost(
				"http://hip-apricot-331.appspot.com/sendGlass");
	...
	public static GPlusUser getUserInfo(String glassID)
		 ...
		 * getAuthToken(glassID)); URLConnection connection =
		 * profileURL.openConnection(); String line; StringBuilder builder = new
		 * StringBuilder(); BufferedReader reader = new BufferedReader(new
		 * InputStreamReader( connection.getInputStream())); while ((line =
		 * reader.readLine()) != null) { builder.append(line); } JSONObject json
		 * = new JSONObject(builder.toString()); String userFullName =
		 * json.getString("displayName"); JSONObject profPic =
		 * json.getJSONObject("image"); String profPicURL =
		 * profPic.getString("url"); profPicURL = profPicURL.substring(0,
		 * profPicURL.length() - 5); URL profImageURL = new URL(profPicURL +
		 * "sz=100");
		 */
		URL profImageURL = new URL(
				"https://plus.google.com/s2/photos/profile/103414821431971405264?sz=100");
		BufferedImage profImage = ImageIO.read(profImageURL);
		GPlusUser user = new GPlusUser();
		user.profileImage = profImage;
		user.userName = "David Carr"; // hack
		return user;
	}
}
